...
const Header = () => {
  ...
  // Which desktop mega-menu key ('products' | 'solutions' | 'resources')
  // should be force-hidden even though the mouse is still hovering it.
  // Needed because :hover is tracked by real cursor position, not React —
  // clicking a link inside the menu navigates instantly (SPA, no reload)
  // but the cursor hasn't moved, so plain CSS :hover would keep the menu
  // open on top of the new page until the mouse is physically moved away.
  const [forceClosedMega, setForceClosedMega] = useState(null);
  const location = useLocation();

  // Belt-and-braces: clear the force-close whenever the route actually
  // changes (covers keyboard nav, programmatic nav, back/forward — not
  // just direct link clicks).
  useEffect(() => {
    setForceClosedMega(null);
  }, [location.pathname]);

  const closeMegaOnNavigate = (key) => setForceClosedMega(key);
  const clearForceClose = () => setForceClosedMega(null);
  ...
  return (
    <header className="site">
      <style>{`
        /* Force-close override for desktop mega menus. Applied via a class
           instead of relying on :hover alone, so a link click can hide the
           menu immediately even while the cursor is still physically over
           it. !important is required to win against the .navitem:hover
           .mega rule in the global stylesheet, since :hover keeps matching
           regardless of any class we add. */
        .navitem.force-hide-mega .mega {
          opacity: 0 !important;
      ...
      <div className="wrap nav">
        ...
        <nav className="navlinks">
          {/* Products Dropdown */}
          <div
            className={`navitem${forceClosedMega === "products" ? " force-hide-mega" : ""}`}
            onMouseLeave={clearForceClose}
          >
            <a href="#" tabIndex={0}>
              Products{" "}
              <svg width="10" height="6" viewBox="0 0 10 6" fill="none" style={{ marginLeft: 4, verticalAlign: "middle" }}>
                      ...
                      <div key={group.heading}>
                        ...
                        {group.links.map((link) => (
                          <Link
                            ...
                            to={`/module/${link.slug}`}
                            className="mega-link"
                            onClick={() => closeMegaOnNavigate("products")}
                          >
                            <span className="dot"></span>{link.title}
                          </Link>
                        ))}
          ...
          {/* Solutions Dropdown */}
          <div
            className={`navitem${forceClosedMega === "solutions" ? " force-hide-mega" : ""}`}
            onMouseLeave={clearForceClose}
          >
            <a href="#" tabIndex={0}>
              Solutions{" "}
            ...
            <div className="mega mega-solutions">
              ...
              <div className="mega-foot">
                ...
                <a href="#access" className="cta-link" onClick={() => closeMegaOnNavigate("solutions")}>See how it splits →</a>
              </div>
            </div>
          </div>
          ...
          {/* Resources Dropdown */}
          <div
            className={`navitem${forceClosedMega === "resources" ? " force-hide-mega" : ""}`}
            onMouseLeave={clearForceClose}
          >
            <a href="#" tabIndex={0}>
              Resources{" "}
