...
//
// This component no longer hardcodes any module's content, and it no
// longer reads the URL hash by hand either — now that Header.jsx links
// with react-router's <Link to={`/module/${slug}`}>, this component
// just asks the router for the current :slug via useParams().
...
/* ---------------------------------------------------------------------
   Small shared icon set (one dictionary of raw SVG path strings, one
   component that drops them into an svg — avoids writing 15 near-
   identical icon components by hand).
--------------------------------------------------------------------- */
const ICON_PATHS = {
...
const Module = () => {
    // useParams() reads whatever the router matched for :slug in the
    // <Route path="/module/:slug"> — e.g. visiting /module/inventory
    // makes slug === "inventory". No manual URL parsing needed anymore.
    const { slug } = useParams();

    // openFaq: which FAQ item (by index) is expanded. null = none open.
    // This is separate from `slug` on purpose — it's UI state, not data.
    const [openFaq, setOpenFaq] = useState(null);

    // Whenever the user navigates to a *different* module (slug changes),
    // collapse any open FAQ left over from the previous module and jump
    ...
    // Look up the module data. Fall back to procurement so the page
    // never renders blank if an unknown/missing slug shows up.
    ...
    return (
      ...
      `}</style>
            ...
            {/* Hero / breadcrumb — every value here now comes from `m` */}
            ...
            <section className="mod-section">
                <div className="wrap">
                    ...
                    {/* AI strip is optional — not every module has one, so we guard on it */}
                    {m.ai && m.ai.length > 0 && (
                        <div
                            className="mod-wide-grid"
                            style={{ gridTemplateColumns: `repeat(${m.ai.length}, 1fr)` }}
                        >
                            {m.ai.map(([title, desc], i) => (
                                <div
                                    className="mod-wide-card"
                                    key={title}
                                    style={i > 0 ? { borderLeft: "1px solid rgba(30,30,30,0.08)" } : undefined}
                                >
                                    <h4 className="mod-wide-title">{title}</h4>
                                    <p className="mod-wide-desc">{desc}</p>
                                </div>
                            ))}
                        </div>
                    )}
            ...
            <section className="mod-section mod-centered">
                <div className="wrap">
                    ...
                    <p className="mod-section-lede">
                        ...
                        run it from the web dashboard — both reading and writing the same real-time ledger.
                    </p>

                    <div className="mod-split">
                        <div className="mod-split-card">
                            <span className="mod-tag yellow">MOBILE APP</span>
                            <h3 className="mod-split-h">On the floor</h3>
                            <p className="mod-split-desc">
                                Offline-first — actions queue on the device and sync automatically the moment connectivity returns.
                            </p>
            ...
            {/* FAQs — this section owns its own bit of state: which item is open */}
            <section className="mod-section mod-centered">
                <div className="wrap">
            ...
            {/* Related modules — real routes, same pattern the nav uses */}
            <section className="mod-section mod-centered">
                <div className="wrap">
